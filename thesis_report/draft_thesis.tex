...
used a variational Bayes approximation of the posterior distribution;\cite{blei2003latent} alternative inference techniques use Gibbs sampling and
...
\For{$t = 0 to \infty$}{\label{forins}
	E step:
	
	Initialize $\gamma_{tk} = 1$
	
	\Repeat{$\frac{1}{K} \sum_k$ |change in $\gamma_{tk}$| $< 0.00001$}{
	...
	Compute $\lambda_{kw}* = \eta + Dn_{tw}\phi_{twk}$
	
	Set $\lambda = (1 - \rho_t)\lambda + \rho_{t}\lambda*$
}
