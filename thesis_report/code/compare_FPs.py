...
# Choose the Framework Programme
parser.add_argument('dataset', action='store', choices=['FP4', 'FP5', 'FP6', 'FP7', 'H2020'], help='input dataset')
...
def get_topic_vectors(topic_eu_id, topic_usa_id):
    ...
    eu_topic_terms_tuples = ldaEU.show_topic(topic_eu_id, topn=ldaEU.num_terms)
    us_topic_terms_tuples = ldaUSA.show_topic(topic_usa_id, topn=ldaUSA.num_terms)
...
# Create an empty dataframe for later use, with rows: EU topics, columns: USA topics
eu = [('eu' + str(i)) for i in range(10)]
usa = [('usa' + str(i)) for i in range(10)]
dfJSD = pd.DataFrame(index=eu, columns=usa)

# Compare the topics bu calculating the JSD
